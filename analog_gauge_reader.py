import cv2
import numpy as np
import paho.mqtt.client as mqtt # Version 1.5.1
import time
...
def main():
    ...
    # Feed an image (or frame) to get the current value, based on the calibration, by default uses the same image as calibration
    img = cv2.imread(folder_path + "/" + filename + ".jpg")
    ...
    # MQTT Konfiguration
    mqtt_broker = "192.168.178.132"  # Setzen Sie die IP-Adresse Ihres MQTT-Brokers ein
    mqtt_port = 1883  # Setzen Sie den entsprechenden Port ein
    mqtt_topic = "measurement/gauge_value"  # Setzen Sie das gewünschte MQTT-Thema ein
    mqtt_username = "admin"  # Setzen Sie Ihren Benutzernamen ein
    mqtt_password = "1234567"  # Setzen Sie Ihr Passwort ein

    # MQTT-Verbindung initialisieren
    client = mqtt.Client()
	  #client.on_connect = on_connect
    client.username_pw_set(mqtt_username, mqtt_password)# Benutzername und Passwort setzen
    client.connect(mqtt_broker, mqtt_port)
    
    data_float = float(val)

    # Round the float to two decimal places
    rounded_float = round(data_float, 2)

    # Convert the rounded float back to a string (optional, if needed)
    rounded_string = str(rounded_float)
    
    # Wert von 'val' als MQTT-Nachricht veröffentlichen
    client.publish(mqtt_topic, str(rounded_string))
